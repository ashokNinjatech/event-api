...
from app.api.schemas import ErrorModel
...
class EventService:
    ...
    def get_events_by_date_range(self, start_date: str, end_date: str) -> List[dict]:
        ...
        if end_datetime < start_datetime:
            error_msg = "End date cannot be less then start date"
            logger.info("Event failed: %s",str(error_msg))
            response = ErrorModel(code="400", msg=error_msg)
        ...
        try:
            ...
            error = str(e)
            logger.info("Event failed: %s", str(error))
            response = ErrorModel(code="500", msg=error)
            return response
